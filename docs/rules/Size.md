...
The templates from this rule serve as message prefixes:

```php
v::size('GB', v::between(1, 2))->assert('video.mp4');
// The size in gigabytes of "video.mp4" must be between 1 and 2

v::not(v::size('MB', v::greaterThan(4)))->assert('image.png');
// The size in megabytes of "image.png" must not be greater than 4
```
